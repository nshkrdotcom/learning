...
def append_alias(project: Project, run_id: str, experiment_id: str | None, slug: str) -> None:
    path = _alias_cache_path(project, for_write=True)
...
def read_aliases(project: Project) -> tuple[list[AliasRecord], bool]:
    path = _alias_cache_path(project)
    if not path.exists():
        return [], False
...
def resolve_run_id(project: Project, alias: str) -> str:
    if (project.runs_dir / alias / "run.json").exists():
        return alias
    records, malformed = read_aliases(project)
    if malformed:
        _write_rebuild_marker(project)
    if not records:
    ...
    by_prefix = [
        record.run_id for record in records if record.run_id.lower().startswith(alias_lower)
    ]
    by_component = [
        record.run_id
        for record in records
        if f"{record.experiment_id}_{record.slug}".lower().startswith(alias_lower)
    ]
...
def _alias_cache_path(project: Project, *, for_write: bool = False) -> Path:
    primary = project.mechledger_dir / "alias_cache.txt"
    if not for_write:
        if primary.exists():
            return primary
        fallback = _temp_cache_root(project) / "alias_cache.txt"
        return fallback if fallback.exists() else primary
    if _can_write(project.mechledger_dir):
        return primary
    return _temp_cache_root(project) / "alias_cache.txt"


def _write_rebuild_marker(project: Project) -> None:
    for root in [project.mechledger_dir / "cache", _temp_cache_root(project) / "cache"]:
        try:
            root.mkdir(parents=True, exist_ok=True)
            (root / "alias_rebuild_required").write_text("1\n", encoding="utf-8")
            return
        except OSError:
            continue
...
def _temp_cache_root(project: Project) -> Path:
    import hashlib

    digest = hashlib.sha256(str(project.root).encode()).hexdigest()[:12]
    return Path(tempfile.gettempdir()) / f"mechledger_cache_{digest}"
