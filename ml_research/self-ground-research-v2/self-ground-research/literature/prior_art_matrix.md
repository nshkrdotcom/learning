...
**Free check before anything else (decision D010):** `pythia-70m-deduped`
SAEs are hosted on Neuronpedia under the `p70d-sm` release family
(`ctigges/pythia-70m-deduped__res-sm_processed`), with `res-sm` variants
confirmed at least at layers 0–5 (`blocks.N.hook_resid_post` — the exact
hook-point convention this repo already uses) and at least some auto-interp
explanations already attached. Look up the two feature IDs already on
record from Phase 2 before writing any new code:
`https://www.neuronpedia.org/pythia-70m-deduped/2-res-sm/12300` and
`https://www.neuronpedia.org/pythia-70m-deduped/2-res-sm/25521`. Whatever is
found (a label, or nothing) is real, citable, dated information — record it
here with a date and a URL, not from memory.
...
| Kassner & Schütze — *Negated and Misprimed Probes for Pretrained Language Models* | ACL 2020 | Not about self-explanation or SAEs — cited here because it's the direct motivation for E000 (decision D007): documents that pretrained LMs frequently fail to update predictions correctly under negation. Directly relevant to whether the behavioral premise of this whole case study holds on a 70M-parameter model; verify against the real E000 result rather than assuming the literature's finding transfers. |
