...
class EvidenceGraphService:
    ...
    def query(self, kind: str, ref: str) -> dict[str, Any]:
        ...
        return {"status": "ok", "query": kind, "ref": ref, "count": len(results), "results": results}
    ...
    def _collect_run_edges(self, edges: list[EvidenceEdge]) -> None:
        ...
        for run_dir in sorted(runs_dir.glob("*")) if runs_dir.exists() else []:
            ...
            self._collect_blocker_edges(edges, run_ref, blocker_report, run_dir / "blocker_report.json")
    ...
    def _collect_blocker_edges(
        ...
        if not blocker_report:
            return
        blocker_ref = str(blocker_report.get("wb_ref") or stable_ref("blocker", run_ref, blocker_report))
